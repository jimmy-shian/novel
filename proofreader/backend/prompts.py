...
def extract_characters_prompt(text: str, rag_context: str = "") -> list[dict]:
    system = dedent("""
        ...
        2. **抽取標準**：只記錄有具體姓名、或對當前及未來劇情有重大推動作用的角色。
        3. **資訊格式**：
            - 「角色名稱」：正式名稱（繁體）。
            - 「別名」：外號、曾用名。
            - 「身份」：頭銜、所屬勢力。
...
def merge_characters_prompt(existing_chars_json: str, new_chars_json: str) -> list[dict]:
    system = dedent("""
        ...
        整合與去重規則：
        1. **識別重複與合併**：如果新資料與總表描述的是同一人，必須合併。
        2. **處理姓名變體/錯別字**：特別注意「音同字不同」的變體或常見錯字（例如「穆白」與「慕白」）。如果背景資料、人際關係或描述內容顯示為同一人，**必須視為同一個角色**，並統一人選中「最正確」的寫法作為主名稱。
        3. **標準化人際關係**：在「人際關係」欄位中，提到的其他角色也必須統一人名（例如將關係中的「慕白」改為標準的「穆白」）。
        4. **清理雜質**：如果新資料中仍混入了「路人」或「背景板角色」（無姓名、無主線價值），請在此步驟將其徹底刪除。
        5. **全繁體中文**：所有輸出內容必須使用繁體中文。
        6. 只輸出 JSON 陣列。
    """).strip()
...
def extract_events_prompt(text: str, chapter: str = "") -> list[dict]:
    system = dedent("""
        你是一位精通小說敘事結構的分析助手。
        你的任務是從「最新章節」中抽取對整體劇情有意義的「重要事件」。
        
        重要指令：
        - **篩選標準**：記錄對主角成長、勢力變動、重大衝突、或核心伏筆揭露有貢獻的事件。
        - **排除瑣事**：跳過日常對話、無關緊要的過場、一般的環境描寫。
        - **精簡描述**：每項描述約 30-80 字，繁體中文。
        - **避免重複**：只輸出該章節新發生的內容。
